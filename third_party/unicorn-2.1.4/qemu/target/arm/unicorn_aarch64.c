                ...
                size_t *size)
{
    ...
    } else if (regid == UC_ARM64_REG_SPSR_EL1) { // hollywood_emu patch
        CHECK_REG_TYPE(uint64_t);
        *(uint64_t *)value = env->banked_spsr[1]; /* BANK_SVC = SPSR_EL1 */
    } else {
        switch (regid) {
        default:
            break;
                 ...
                 size_t *size, int *setpc)
{
    ...
    if (regid >= UC_ARM64_REG_X0 && regid <= UC_ARM64_REG_X28) {
        ...
        CHECK_REG_TYPE(uint64_t);
        env->banked_spsr[1] = *(uint64_t *)value; /* BANK_SVC = SPSR_EL1 */
    } else {
        switch (regid) {
        default:
            break;
