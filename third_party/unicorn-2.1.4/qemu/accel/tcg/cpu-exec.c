...
static inline bool cpu_handle_exception(CPUState *cpu, int *ret)
{
    ...
    if (cpu->exception_index >= EXCP_INTERRUPT) {
        ...
        *ret = cpu->exception_index;
        ...
        env->nip += 4;
#endif
        // Unicorn: call registered interrupt callbacks (observability only)
        HOOK_FOREACH_VAR_DECLARE;
        ...
        /* hollywood_emu: deliver the CPU exception through the guest's own vector
         * table (arm_cpu_do_interrupt sets SPSR/ELR/ESR/FAR/PSTATE and PC=VBAR+off)
         * instead of stopping emulation. This lets the guest OS run its real
         * fault / BUG / WARN / syscall / IRQ handlers -- essential for booting a
         * full OS. */
        {
            CPUClass *cc = CPU_GET_CLASS(cpu);
            cc->do_interrupt(cpu);
